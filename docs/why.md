...
`go-ruby-openbao/openbao` reimplements Ruby's `openbao` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-openbao/openbao` directly, with no Ruby runtime;
...
Because `openbao` is pure Go with cgo disabled, it also compiles to **WebAssembly**
